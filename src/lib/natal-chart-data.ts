...
export interface CalculatedAspect {
  ...
  planet2: Planet
  aspect: Aspect
  orb: number // точность аспекта в градусах (чем меньше, тем сильнее)
  interpretation: string
  strength: "точный" | "сильный" | "умеренный"
}

export function calculateAspects(positions: PlanetPosition[]): CalculatedAspect[] {
  const result: CalculatedAspect[] = []
  ...
  for (let i = 0; i < positions.length; i++) {
    for (let j = i + 1; j < positions.length; j++) {
      ...
      // Абсолютный угол между планетами
      const angle1 = p1.signIndex * 30 + p1.degree
      const angle2 = p2.signIndex * 30 + p2.degree
      let diff = Math.abs(angle1 - angle2)
      if (diff > 180) diff = 360 - diff

      for (const aspect of aspects) {
        const orb = aspectOrbs[aspect.name] || 6
        const angleDiff = Math.abs(diff - aspect.angle)
        if (angleDiff <= orb) {
          const strength: CalculatedAspect["strength"] =
            angleDiff <= 2 ? "точный" : angleDiff <= 4 ? "сильный" : "умеренный"
          result.push({
            planet1: p1.planet,
            planet2: p2.planet,
            aspect,
            orb: angleDiff,
            strength,
            interpretation: getAspectInterpretation(p1.planet, p2.planet, aspect),
          })
          break // только один аспект на пару
        }
      }
    }
  }

  // Сортировка: сначала точные, потом по силе
  return result.sort((a, b) => {
    const order = { "точный": 0, "сильный": 1, "умеренный": 2 }
    return order[a.strength] - order[b.strength]
  })
}

function getAspectInterpretation(p1: Planet, p2: Planet, aspect: Aspect): string {
  const typeText = aspect.type === "harmonious"
    ? "Гармоничное взаимодействие"
    : aspect.type === "tense"
    ? "Напряжённое взаимодействие"
    : "Слияние энергий"

  const flowText = aspect.type === "harmonious"
    ? `энергии ${p1.name} и ${p2.name} поддерживают друг друга, создавая естественный поток`
    : aspect.type === "tense"
    ? `энергии ${p1.name} и ${p2.name} конфликтуют, создавая внутреннее напряжение, которое требует проработки`
    : `энергии ${p1.name} и ${p2.name} сливаются, взаимно усиливая друг друга`

  const specific = getSpecificAspectText(p1.id, p2.id, aspect)

  return `${typeText}. ${p1.symbol}${p1.name} и ${p2.symbol}${p2.name} в ${aspect.name.toLowerCase()}: ${flowText}. ${specific}`
}

function getSpecificAspectText(id1: string, id2: string, aspect: Aspect): string {
  const pair = [id1, id2].sort().join("-")
  const isTense = aspect.type === "tense"
  const isHarmonious = aspect.type === "harmonious"
  const p1Name = id1.charAt(0).toUpperCase() + id1.slice(1)
  const p2Name = id2.charAt(0).toUpperCase() + id2.slice(1)

  const interpretations: Record<string, { tense: string; harmonious: string; neutral: string }> = {
    "mercury-sun": {
      tense: "Ваш ум может спорить с вашим эго — между тем, что вы думаете, и тем, кем вы хотите быть, есть конфликт.",
      harmonious: "Ваш ум и ваша суть работают в гармонии — вы ясно мыслите и ярко выражаете себя.",
      neutral: "Ваш разум и ваше Я слиты — вы мыслите ярко и уверенно, ваше слово имеет вес.",
    },
    "moon-sun": {
      tense: "Ваши эмоции (Луна) и ваше сознание (Солнце) в конфликте — то, что вы чувствуете, расходится с тем, чего хотите. Внутреннее напряжение между потребностью и волей.",
      harmonious: "Глубокая гармония между вашими чувствами и вашей волей — вы цельная личность, ваши эмоции поддерживают ваши цели.",
      neutral: "Слияние сознания и подсознания — вы глубоко чувствуете то, что думаете, и ясно осознаёте то, что чувствуете.",
    },
    "mars-sun": {
      tense: "Ваша воля и ваша энергия могут конфликтовать — вы хотите одного, а действуете иначе. Гнев может подрывать ваши цели.",
      harmonious: "Ваша энергия (Марс) прекрасно поддерживает вашу жизненную цель (Солнце) — вы действуйте решительно и целенаправленно.",
      neutral: "Слияние воли и действия — вы воплощаете свои желания немедленно, ваша сила равна вашей решимости.",
    },
    "mars-mercury": {
      tense: "Ваш ум и ваша агрессия спорят — слова могут быть резкими, вы можете говорить прежде чем думать.",
      harmonious: "Острый ум и решительные действия работают вместе — вы быстро думаете и быстро действуете.",
      neutral: "Слияние мысли и действия — вы мыслите как воин, ваши слова имеют силу удара.",
    },
    "mars-venus": {
      tense: "Страсть и нежность в конфликте — то, чего вы хотите физически, может расходиться с тем, что вам нужно эмоционально.",
      harmonious: "Гармония желания и любви — вы страстны и нежны одновременно, ваша сексуальность и чувствительность сбалансированы.",
      neutral: "Слияние любви и страсти — вы любите всем существом, ваши чувства и желания неразделимы.",
    },
    "mars-moon": {
      tense: "Ваши эмоции и ваша агрессия в конфликте — гнев может прорываться неожиданно, эмоции могут вести к импульсивным действиям.",
      harmonious: "Эмоциональная сила и воля в гармонии — вы действуете из глубины чувств, ваши инстинкты ведут вас верно.",
      neutral: "Слияние эмоций и действия — вы чувствуете и действуете одновременно, ваши реакции мгновенны.",
    },
    "mercury-moon": {
      tense: "Ваши чувства и ваш разум спорят — вы думаете одно, а чувствуете другое. Трудно выразить эмоции словами.",
      harmonious: "Глубокая связь ума и сердца — вы ясно понимаете свои чувства и умеете их выражать. Эмоциональный интеллект.",
      neutral: "Слияние мысли и чувства — вы мыслите эмоционально и чувствуете осознанно.",
    },
    "saturn-sun": {
      tense: "Ваша воля сталкивается с ограничениями — вы чувствуете, что мир сдерживает вас. Отец, авторитет, структура могут давить. Урок — дисциплина.",
      harmonious: "Ваша жизненная сила поддерживается дисциплиной — вы способны упорно строить свою жизнь на прочном фундаменте.",
      neutral: "Слияние воли и структуры — вы воплощаете порядок, ваша сила в дисциплине и ответственности.",
    },
    "saturn-moon": {
      tense: "Ваши эмоции сдерживаются — вам трудно выражать чувства, вы можете казаться холодным. Эмоциональная депривация в детстве. Урок — доверять.",
      harmonious: "Эмоциональная стабильность и зрелость — вы способны контейнировать свои чувства и других. Надёжная забота.",
      neutral: "Слияние эмоций и структуры — вы чувствуете ответственно, ваша забота дисциплинирована.",
    },
    "jupiter-sun": {
      tense: "Избыток оптимизма может вести к обещаниям, которые вы не можете сдержать. Чрезмерное расширение может истощить.",
      harmonious: "Ваша жизненная сила усиливается оптимизмом и мудростью — вы естественным образом растёте и расширяетесь.",
      neutral: "Слияние воли и мудрости — вы воплощаете оптимизм, ваша жизненная сила притягивает удачу.",
    },
    "jupiter-moon": {
      tense: "Эмоциональный избыток — вы можете переедать, переоценивать, быть сентиментальным. Опасность зависимостей.",
      harmonious: "Эмоциональная щедрость и оптимизм — вы естественно тёплый, заботливый, мудрый в чувствах.",
      neutral: "Слияние эмоций и мудрости — вы чувствуете широко, ваша эмпатия философская.",
    },
    "saturn-mars": {
      tense: "Ваша энергия блокируется ограничениями — вы можете чувствовать, что ваши действия постоянно тормозятся. Урок — терпение и стратегия.",
      harmonious: "Дисциплинированная энергия — вы способны направлять свою силу упорно и стратегически. Достижение через упорство.",
      neutral: "Слияние действия и структуры — вы действуете методично, ваша сила в планировании.",
    },
    "uranus-sun": {
      tense: "Ваша потребность в свободе конфликтует с вашей потребностью в стабильности — внезапные перемены могут разрушать ваши цели.",
      harmonious: "Оригинальность и новизна поддерживают вашу жизненную силу — вы естественно инновационны и свободны.",
      neutral: "Слияние воли и революции — вы воплощаете перемены, ваша сила в нестандартности.",
    },
    "uranus-moon": {
      tense: "Эмоциональная нестабильность — ваши чувства могут меняться внезапно, потребность в свободе борется с потребностью в безопасности.",
      harmonious: "Эмоциональная свобода и оригинальность — вы естественно независимы в чувствах, открытый новым опытам.",
      neutral: "Слияние эмоций и новизны — вы чувствуете нестандартно, ваши реакции неожиданные.",
    },
    "neptune-sun": {
      tense: "Иллюзии о себе — вы можете не видеть себя ясно, идеализировать или обманывать себя. Урок — честность с собой.",
      harmonious: "Духовная чувствительность и творчество поддерживают вашу суть — вы естественно вдохновенны и сострадательны.",
      neutral: "Слияние воли и духовности — вы воплощаете мечту, ваша сила в видении.",
    },
    "neptune-moon": {
      tense: "Эмоциональная чувствительность может быть чрезмерной — вы можете теряться в чужих эмоциях, впадать в фантазии или зависимости.",
      harmonious: "Глубокая духовная эмпатия — вы естественно чувствуете тонкие миры, ваша сострадательность целительна.",
      neutral: "Слияние эмоций и мистики — вы чувствуете запредельное, ваши эмоции духовны.",
    },
    "pluto-sun": {
      tense: "Глубинная борьба за власть — вы можете сталкиваться с кризисами идентичности, потребность контроля может разрушать.",
      harmonious: "Глубинная сила трансформации — вы способны возрождаться из пепла, ваша воля имеет магнетическую силу.",
      neutral: "Слияние воли и трансформации — вы воплощаете силу, ваша суть — возрождение.",
    },
    "pluto-moon": {
      tense: "Эмоциональные кризисы и трансформации — глубокие страхи, потери, периоды эмоциональной смерти и возрождения.",
      harmonious: "Глубокая эмоциональная сила — вы способны трансформировать через чувства, ваша эмпатия имеет целительную силу.",
      neutral: "Слияние эмоций и трансформации — вы чувствуете на глубине души, ваши эмоции меняют вас и других.",
    },
    "mercury-venus": {
      tense: "Ваши слова и ваши ценности могут спорить — вы говорите одно, а цените другое.",
      harmonious: "Красота слова — вы естественно дипломатичны, эстетичны, ваши слова приятны и ценны.",
      neutral: "Слияние мысли и любви — вы мыслите прекрасным, ваши слова — искусство.",
    },
    "jupiter-saturn": {
      tense: "Расширение и ограничение в конфликте — вы хотите расти, но структура сдерживает, или структура есть, но нет роста.",
      harmonious: "Баланс роста и структуры — вы способны расширяться организованно, ваша амбиция мудрая.",
      neutral: "Слияние expansion и ограничения — вы строите масштабно, ваша мудрость практична.",
    },
  }

  const interp = interpretations[pair]
  if (interp) {
    return isTense ? interp.tense : isHarmonious ? interp.harmonious : interp.neutral
  }

  // Общая интерпретация для пар без специфической
  return `${p1Name} и ${p2Name} в ${aspect.name.toLowerCase()} — взаимодействие этих энергий создаёт уникальную динамику в вашей карте.`
}
...
export interface KeyDate {
  ...
  title: string
  planet: string
  type: "return" | "aspect" | "transit"
  description: string
  significance: "высокая" | "средняя" | "низкая"
}
...
export function calculateKeyDates(
  ...
  const addDate = (age: number, title: string, planet: string, type: KeyDate["type"], desc: string, sig: KeyDate["significance"]) => {
  ...
  // Юпитер-возврат (каждые ~12 лет)
  addDate(12, "Первый возврат Юпитера", "♃ Юпитер", "return",
    "Начало нового 12-летнего цикла роста. В этом возрасте формируется мировоззрение, появляются новые возможности для обучения и расширения горизонтов. Важно быть открытым новому опыту.",
    "средняя")
  addDate(24, "Второй возврат Юпитера", "♃ Юпитер", "return",
    "Завершение цикла молодости, начало взрослого расширения. Время амбиций, карьерного роста, поиска смысла. Вы готовы взять на себя больше ответственности за свою жизнь.",
    "средняя")
  addDate(36, "Третий возврат Юпитера", "♃ Юпитер", "return",
    "Начало цикла зрелости. Переоценка ценностей, новые направления роста. Многие в этом возрасте меняют карьеру или находят новое призвание.",
    "средняя")
  addDate(48, "Четвёртый возврат Юпитера", "♃ Юпитер", "return",
    "Цикл мудрости. Расширение через передачу опыта, наставничество. Время собрать урожай и поделиться знаниями с новыми поколениями.",
    "средняя")
  addDate(60, "Пятый возврат Юпитера", "♃ Юпитер", "return",
    "Начало цикла поздней зрелости. Философское осмысление жизни, духовное расширение. Время для путешествий, обучения новому, реализации давних мечтаний.",
    "средняя")

  // Сатурн-возврат (каждые ~29.5 лет)
  addDate(29, "Первый возврат Сатурна", "♄ Сатурн", "return",
    "Один из важнейших жизненных переходов. Космический экзамен на зрелость. Время структурировать жизнь, взять ответственность, определиться с карьерой и долгосрочными целями. Может быть кризисом, но после — твёрдая опора на десятилетия.",
    "высокая")
  addDate(59, "Второй возврат Сатурна", "♄ Сатурн", "return",
    "Второй космический экзамен. Переоценка достижений, подготовка к следующему этапу. Время передать опыт, завершить незаконченное, подготовить фундамент для поздней зрелости.",
    "высокая")

  // Уран-полу方形 (~42 года)
  addDate(42, "Оппозиция Урана (кризис середины жизни)", "♅ Уран", "aspect",
    "Знаменитый 'кризис середины жизни'. Глубокая потребность в переменах, свободе, переоценке. Время избавиться от того, что больше не служит вам. Может быть разрушительным или освобождающим — зависит от готовности к переменам.",
    "высокая")

  // Возврат Урана (~84 года)
  addDate(84, "Возврат Урана", "♅ Уран", "return",
    "Полный цикл Урана. Редкая точка, которую достигают не все. Время духовной свободы, мудрости, освобождения от земных привязанностей. Может быть периодом великого духовного прозрения.",
    "высокая")

  // Квадрат Нептуна (~41 год)
  addDate(41, "Квадрат Нептуна", "♆ Нептун", "aspect",
    "Время духовного переосмысления. Иллюзии рассеиваются,梦想 сталкиваются с реальностью. Может быть периодом сомнений, разочарований, но и глубокого духовного пробуждения. Важно найти якорь в реальности.",
    "средняя")

  // Оппозиция Нептуна (~84 года, редко)
  addDate(84, "Оппозиция Нептуна", "♆ Нептун", "aspect",
    "Духовная зрелость. Подготовка к переходу, слияние с божественным. Время прощения, отпускания, подготовки к возвращению домой.",
    "средняя")

  // Квадрат Плутона (~35-40 лет)
  addDate(37, "Квадрат Плутона", "♇ Плутон", "aspect",
    "Глубинная трансформация. Время, когда старые структуры разрушаются, чтобы освободить место для новых. Может быть периодом потери и возрождения. После — вы становитесь сильнее и глубже.",
    "высокая")

  // Оппозиция Плутона (~45-50 лет для поколений 1960-х)
  addDate(45, "Оппозиция Плутона", "♇ Плутон", "aspect",
    "Глубинное противостояние. Столкновение с собственной тенью, потребность во власти и контроле. Время интеграции тёмной стороны, иначе она будет управлять вами.",
    "средняя")

  // Возврат Хирона (~50 лет) — "раненый целитель"
  addDate(50, "Возврат Хирона", "⚷ Хирон", "return",
    "Время исцеления глубинных ран. 'Раненый целитель' возвращается к своей ране, чтобы исцелить её окончательно. После этого периода вы можете стать целителем для других через свой опыт.",
    "высокая")

  // Узлы Луны — возврат (~18.6 лет)
  addDate(19, "Возврат Лунных Узлов", "☊ Узлы", "return",
    "Начало нового цикла кармического роста. Переоценка направления жизни. Время прислушаться к интуиции и понять, куда движется ваша душа.",
    "средняя")
  addDate(38, "Второй возврат Лунных Узлов", "☊ Узлы", "return",
    "Середина кармического цикла. Важная точка выбора — продолжать ли прежний путь или развернуться. Время прислушаться к зову души.",
    "средняя")
  addDate(56, "Третий возврат Лунных Узлов", "☊ Узлы", "return",
    "Завершение большого кармического цикла. Подведение итогов, осмысление пройденного. Время отпустить незавершённое и подготовиться к следующему циклу.",
    "средняя")

  // Сортировка по возрасту
  return dates.sort((a, b) => a.age - b.age)
}
