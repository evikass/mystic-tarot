...
export function PalmInteractive() {
  ...
  const renderInteractivePalm = () => {
    ...
    const lines: Record<string, string> = {
      ...
      // Линия Жизни: начинается между большим и указательным пальцем (~95,148),
      // идёт ВНИЗ-ВЛЕВО дугой, огибая бугор Венеры (основание большого пальца),
      // заканчивается у запястья. Образует C-образную дугу вокруг большого пальца.
      life: "M 95 148 C 60 160 42 200 40 255 C 38 310 55 340 82 352",
      // Линия Головы: начинается рядом с линией Жизни (~95,165),
      // идёт горизонтально ВПРАВО через ладонь
      head: "M 95 168 Q 155 158 232 178",
      // Линия Сердца: выше Головы, ближе к основаниям пальцев,
      // слегка изгибается сверху
      heart: "M 85 142 Q 160 128 245 140",
      // Линия Судьбы: вертикально от запястья вверх к среднему пальцу
      fate: "M 152 358 L 148 138",
      // Линия Солнца: вертикально параллельно, к безымянному
      sun: "M 188 358 L 185 145",
      // ДОПОЛНИТЕЛЬНЫЕ
      mars: "M 100 155 C 75 170 60 205 58 255 C 56 305 70 335 92 348",
      health: "M 178 355 Q 172 285 168 215",
      marriage: "M 252 140 L 255 156 M 247 143 L 250 159 M 257 136 L 260 152",
      children: "M 250 130 L 252 123 M 254 133 L 256 126 M 258 128 L 260 121",
      travel: "M 220 320 Q 230 328 240 322 M 225 335 Q 235 343 245 337",
      intuition: "M 235 310 Q 195 250 168 200",
      "girdle-venus": "M 92 130 Q 160 110 230 128",
      simian: "M 90 155 Q 160 145 235 160",
      "ring-solomon": "M 96 125 Q 108 112 120 125",
      "ring-saturn": "M 130 128 Q 142 115 154 128",
    }
    ...
    return (
      <svg viewBox="0 0 300 420" className="w-full h-full max-w-xs mx-auto">
        ...
        {/* Правая ладонь, вид ладонью к зрителю.
            Большой палец — слева, отходит от нижней трети ладони под углом ~45°.
            4 пальца сверху: средний (самый длинный) > безымянный ≈ указательный > мизинец (самый короткий).
            Контур начинается от запястья, идёт против часовой стрелки. */}
        <path
          d="M 110 360
